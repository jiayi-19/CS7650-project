# -*- coding: utf-8 -*-
"""checking model.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1RwDfm94Xn1TWJh1IR48zJ-oIW5ANPX9I
"""

import random
from openai import OpenAI
import json


client = OpenAI(
    base_url="https://api.openai.com/v1",
    api_key= "os.environ['OPENAI_API_KEY'],"
)


def evaluate_consistency(
    prompt: str,
    response_a: str,
    response_b: str,
    evaluator_model: str = "gpt-4o-mini"
) -> dict:
    """
    Returns a dict with exactly these keys:
      - consistent: "Yes" or "No"
      - consistent_reason: explanation why A and B are consistent/inconsistent
      - better_model: "A" or "B" (only if consistent == "Yes")
      - better_model_reason: explanation why that response is better (only if consistent == "Yes")
    """
    system_instructions = """
You are a medical expert evaluator. Do not output any extra fields.
"""
    user_instructions = f"""
Prompt:
{prompt}

Response A:
{response_a}

Response B:
{response_b}

Please reply with **only** a JSON object matching this schema:

{{
  "consistent": "<Yes or No>",
  "consistent_reason": "<brief explanation of why the responses are consistent or not>",
  // If "consistent" == "Yes", ALSO include:
  "better_model": "<A or B>",
  "better_model_reason": "<brief explanation why that response is better>"
}}
"""
    completion = client.chat.completions.create(
        model=evaluator_model,
        messages=[
            {"role": "system", "content": system_instructions},
            {"role": "user",   "content": user_instructions}
        ],
        temperature=0.0
    )
    text = completion.choices[0].message.content.strip()

    try:
        result = json.loads(text)
    except json.JSONDecodeError:
        result = {
            "consistent": None,
            "consistent_reason": text,
        }
    return result

examples = [
    {
        "prompt": "What is the description of the medical code F19.139 in ICD10CM?",
        "response_a": (
            "Answer: Option A. Other psychoactive substance abuse with withdrawal, unspecified.\n"
            "Explanation: In ICD‑10‑CM, ‘F19’ covers disorders due to multiple or other psychoactive substances. "
            "The ‘.1’ subclass indicates a use disorder with withdrawal, and the final digit ‘9’ denotes unspecified substance. "
            "Hence F19.139 is exactly withdrawal from an unspecified psychoactive substance."
        ),
        "response_b": (
            "Answer: Option A. Other psychoactive substance abuse with withdrawal, unspecified.\n"
            "Explanation: Chapter F19 relates to psychoactive substance–induced disorders; "
            "code ‘.13’ specifies withdrawal syndrome, and ‘9’ flags unspecified substance type. "
            "Therefore F19.139 correctly refers to withdrawal of an unspecified psychoactive substance."
        ),
    }
]

for ex in examples:
    result = evaluate_consistency(
        ex["prompt"], ex["response_a"], ex["response_b"]
    )
    print(result)





